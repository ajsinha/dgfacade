...
public class ShutdownOrchestrator {
    ...
    public void onContextClosed() {
        ...
        try {
            ...
            // Phase 3: Disconnect brokers
            logShutdownPhase(3, "Disconnect Broker Connections",
                    "Closing Kafka consumers, JMS sessions, and broker connections...");
            phaseDelay();
            logShutdownPhaseComplete(3, "All broker connections closed");

            // Phase 4: Shutdown Execution Engine
            logShutdownPhase(4, "Shutdown Execution Engine",
                    "Terminating Pekko actor system and completing pending handler executions...");
            try {
                executionEngine.shutdown();
                logShutdownPhaseComplete(4, "Pekko actor system terminated");
            } catch (Exception e) {
            ...
            // Phase 5: Close WebSocket sessions
            logShutdownPhase(5, "Close WebSocket Sessions",
                    "Sending close frames to connected WebSocket clients...");
            phaseDelay();
            logShutdownPhaseComplete(5, "All WebSocket sessions closed");

            // Phase 6: Flush logs and release resources
            logShutdownPhase(6, "Flush Logs & Release Resources",
                    "Flushing log buffers and releasing file handles...");
            phaseDelay();
            logShutdownPhaseComplete(6, "Resources released");
        ...
        log.info("║   ✓ Phase 3: Broker connections closed{}║",
                pad("✓ Phase 3: Broker connections closed", 52));
        log.info("║   ✓ Phase 4: Execution engine terminated{}║",
                pad("✓ Phase 4: Execution engine terminated", 52));
        log.info("║   ✓ Phase 5: WebSocket sessions closed{}║",
                pad("✓ Phase 5: WebSocket sessions closed", 52));
        log.info("║   ✓ Phase 6: Resources released{}║",
                pad("✓ Phase 6: Resources released", 52));
        log.info("║                                                                    ║");
